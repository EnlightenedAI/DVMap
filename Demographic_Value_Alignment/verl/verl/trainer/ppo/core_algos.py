        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                id2mean[idx] = torch.mean(scores_tensor)
                                # --- 【核心修改开始】 ---
                # # 1. 计算原始的组内均值
                # raw_mean = torch.mean(scores_tensor)
                
                # # 2. 引入基线修正：取 max(raw_mean, baseline)
                # # 逻辑：
                # # - 如果组内均分是 1.4 (全错)，这里变成 max(1.4, 2.0) = 2.0。
                # #   那么 1.8 分的样本优势 = (1.8 - 2.0)/std = 负数 (受到惩罚)。
                # # - 如果组内均分是 3.5 (做得好)，这里变成 max(3.5, 2.0) = 3.5。
                # #   逻辑回归标准 GRPO，组内拔高个。
                
                # # 注意：确保 baseline 和 scores_tensor 在同一个 device 上
                # baseline_tensor = torch.tensor(ABSOLUTE_BASELINE, device=scores_tensor.device, dtype=scores_tensor.dtype)
                # id2mean[idx] = torch.maximum(raw_mean, baseline_tensor)
...
def compute_grpo_vectorized_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        #         # --- 【核心修改】 ---
        # # 引入基线修正：取 max(raw_mean, baseline)
        # baseline_tensor = torch.tensor(ABSOLUTE_BASELINE, device=scores.device, dtype=scores.dtype)
        
        # # 如果组均分 < 2.0 (全错)，强制提升均值到 2.0，让优势变负
        # # 如果组均分 > 2.0 (有对)，保持原样
        # mean_g = torch.maximum(mean_g, baseline_tensor)
        # # -----------------
        if norm_adv_by_std_in_grpo:
