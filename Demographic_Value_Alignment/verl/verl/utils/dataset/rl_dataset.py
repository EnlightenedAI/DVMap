...
class RLHFDataset(Dataset):
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        raw_prompt = self.processor.apply_chat_template(
                            messages, add_generation_prompt=True, enable_thinking=False,tokenize=False, **apply_kwargs
                        )
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        return len(
                            tokenizer.apply_chat_template(doc[prompt_key],enable_thinking=False, add_generation_prompt=True, **apply_kwargs)
                        )
                    except Exception:
                        print("Error processing one of the samples, skipping...")
                        traceback.print_exc()
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            raw_prompt = self.processor.apply_chat_template(
                messages, add_generation_prompt=True, enable_thinking=False,tokenize=False, **self.apply_chat_template_kwargs
            )
            multi_modal_data = {}
